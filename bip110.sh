...
if [ $# -eq 0 ] || [ "$1" = "-h" ] || [ "$1" = "-help" ]; then
  echo "Interim optional Bitcoin KNOTS+BIP-110 updates between RaspiBlitz releases."
...
downloadDir="/home/$(whoami)/download/bip110"
...
bitcoinLatestVersion=$(curl --header "X-GitHub-Api-Version:2022-11-28" -s https://api.github.com/repos/dathonohm/bitcoin/releases | jq -r '.[].tag_name' | sort | tail -n1 | cut -c 2-)
...
function displayInfo() {
  ...
  echo "# reckless update option (latest Bitcoin KNOTS+BIP-110 release from GitHub)"
...
if [ "${mode}" = "tested" ]; then
  ...
  if [ $# -gt 1 ]; then
    ...
    clear
    echo
    echo "# Update Bitcoin KNOTS+BIP-110 to a chosen version."
    echo
  ...
  if curl --output /dev/null --silent --head --fail \
    ...
    echo "# OK version exists at https://github.com/dathonohm/bitcoin/releases/download/v29.2.knots20251110%2Bbip110-v0.1rc2/${pathVersion}"
    if [ "${mode}" = "custom" ] && [ "$3" = "skipverify" ]; then
      echo "# skipping signature verification"
    fi
    echo "# Press ENTER to proceed to install Bitcoin KNOTS+BIP-110 $bitcoinVersion or CTRL+C to abort."
    read key
  else
...
if [ "${mode}" = "tested" ] || [ "${mode}" = "reckless" ] || [ "${mode}" = "custom" ]; then
  ...
  curl -s "https://api.github.com/repos/dathonohm/guix.sigs/tree/bip110/builder-keys" |
    jq -r '.[].download_url' | while read url; do curl -s "$url" | gpg --import; done

  # download signed binary sha256 hash sum file
  wget --prefer-family=ipv4 --progress=bar:force -O SHA256SUMS https://github.com/dathonohm/guix.sigs/tree/bip110/${bitcoinVersion}/dathonohm/all.SHA256SUMS
  # download the signed binary sha256 hash sum file and check
  wget --prefer-family=ipv4 --progress=bar:force -O SHA256SUMS.asc https://github.com/dathonohm/guix.sigs/tree/bip110/${bitcoinVersion}/dathonohm/all.SHA256SUMS.asc

  if [ "${mode}" = "custom" ] && [ "$3" = "skipverify" ]; then
    echo "# skipping signature verification"
    echo "# display the output of 'gpg --verify all.SHA256SUMS.asc'"
    gpg --verify all.SHA256SUMS.asc
  else
    if gpg --verify all.SHA256SUMS.asc; then
      echo
      echo "****************************************"
      echo "OK --> KNOTS+BIP-110 MANIFEST IS CORRECT"
      echo "****************************************"
      echo
    else
      echo
      ...
      echo "# /home/admin/config.scripts/bip110.sh custom ${bitcoinVersion:-<version>} skipverify"
      exit 1
    fi
  fi
  ...
  if ! sha256sum -c --ignore-missing all.SHA256SUMS; then
    ...
    echo "# FAIL # Downloaded BITCOIN KNOTS+BIP-110 BINARY CHECKSUM:"
    ...
    echo
    echo "# OK --> VERIFIED BITCOIN KNOTS+BIP-110 BINARY CHECKSUM IS CORRECT"
    echo
  fi
fi
...
if [ "${mode}" = "tested" ] || [ "${mode}" = "reckless" ] || [ "${mode}" = "custom" ]; then
  ...
  echo "# OK Bitcoin KNOTS+BIP-110 ${bitcoinVersion} is installed"
  exit 0
